...
import { ResultModalRevenuComponent } from '../result-modal-revenu/result-modal-revenu.component';
...
@Component({
  selector: 'app-drag-drog-revenu',
  templateUrl: './drag-drog-revenu.component.html',
  styleUrls: ['./drag-drog-revenu.component.scss']
})
export class DragDrogRevenuComponent {
  ...
  fetchData(): void {
    ...
    this.faitSuiviRevenuService.getRevenuData(indicateur, axe, startDate.toString(), endDate.toString())
      .subscribe({
        next: (data: [string, number][]) => {
          ...
          const modalRef = this.modalService.open(ResultModalRevenuComponent, {
            size: 'xl',
            centered: true
          });
