...
import { AbonneModel } from './abonne.model';
import { recomendedjob } from './data';
import { DecimalPipe } from '@angular/common';
import { debounceTime, delay, switchMap, tap } from 'rxjs/operators';
import { SortColumn, SortDirection } from './abonne-sortable.directive';

interface SearchResult {
  countries: AbonneModel[];
  total: number;
}
...
function sort(countries: AbonneModel[], column: SortColumn, direction: string): AbonneModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: AbonneModel, term: string, pipe: PipeTransform) {
...
export class AbonneService {
  ...
  private _countries$ = new BehaviorSubject<AbonneModel[]>([]);
