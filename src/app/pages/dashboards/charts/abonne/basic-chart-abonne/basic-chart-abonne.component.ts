...
@Component({
  selector: 'app-basic-chart-abonne',
  templateUrl: './basic-chart-abonne.component.html',
  styleUrls: ['./basic-chart-abonne.component.scss']
})
export class BasicChartAbonneComponent {
  ...
  loadChartData(): void {
    ...
    this.faitSuiviAbonneService.getTypeAbonneInfos(this.startDate, this.endDate).subscribe({
      next: (data) => {
  ...
  private getChartColorsArray(colors: any) {
    ...
    processTypeChartData(data: any[]): void {
      ...
      const postpayes: number[] = [];
      const prepayes: number[] = [];
      const forfaits: number[] = [];
    
      data.forEach(row => {
        moisLabels.push(row[0]);
        postpayes.push(row[1] || 0);
        prepayes.push(row[2] || 0);
        forfaits.push(row[3] || 0);
      });
  ...
  private _basicChart(
    ...
    this.basicChart = {
      series: [
        {
          name: "Abonnés postpayés",
          data: postpayes,
        },
        {
          name: "Abonnés prépayés",
          data: prepayes,
        },
      /*   {
  ...
  generatePDFWithLogo(logoData: string): void {
    ...
    // 🏷️ Titre centré
    const title = 'REPARTITION DES ABONNES';
    ...
    // 📊 Données
    const headers = ['Mois', 'Abonnés facturés', 'Abonnés actifs', 'Abonnés au forfait'];
  ...
  exportToExcel(): void {
    const ws = XLSX.utils.aoa_to_sheet([
      ['Mois', 'Abonnés facturés', 'Abonnés actifs', 'Abonnés au forfait'],
